...
export const TrustBadgesBar = ({ className = '', compact = false }: TrustBadgesBarProps) => {
  if (compact) {
    return (
      <div className={`rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100/90 dark:bg-[#0d0d0d] p-3.5 shadow-sm transition-colors duration-200 ${className}`}>
            ...
            return (
              <div key={idx} className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-[10px] font-extrabold text-zinc-900 dark:text-white uppercase tracking-wider truncate">{badge.title}</span>
                  <span className="text-[9px] text-zinc-500 dark:text-zinc-400 font-normal truncate">{badge.subtitle}</span>
                </div>
              </div>
            );
          })}
          ...
          {badges.map((badge, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center gap-4 text-left w-full max-w-[320px] p-1">
                <div className="w-12 h-12 rounded-xl bg-white dark:bg-zinc-900/90 border border-zinc-200 dark:border-zinc-800/90 flex items-center justify-center text-orange-600 dark:text-orange-400 shrink-0 shadow-sm">
                  <Icon className="w-5.5 h-5.5 text-orange-600 dark:text-orange-400" strokeWidth={1.75} />
                </div>
                <div className="flex flex-col min-w-0">
                  <span className="text-xs sm:text-sm font-extrabold text-zinc-900 dark:text-white uppercase tracking-wider truncate">
                    {badge.title}
                  </span>
